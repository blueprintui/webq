...
import * as elementProperties from './properties.js';
...
describe('elementProperties', () => {
  test('toJSON returns field members only', () => {
    const result = elementProperties.toJSON(makeCtx(), { tagName: 'bp-button' }) as {
      tagName: string;
      properties: { name: string; privacy?: string; readonly?: boolean }[];
    };
    expect(result.tagName).toBe('bp-button');
    expect(result.properties).toHaveLength(1);
    expect(result.properties[0].name).toBe('variant');
    expect(result.properties[0].privacy).toBe('public');
  });

  test('toJSON throws for unknown element', () => {
    expect(() => elementProperties.toJSON(makeCtx(), { tagName: 'bp-unknown' })).toThrow();
  });

  test('toMarkdown returns formatted properties', () => {
    const md = elementProperties.toMarkdown(makeCtx(), { tagName: 'bp-button' });
