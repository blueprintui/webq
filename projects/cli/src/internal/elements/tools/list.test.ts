...
import * as elementList from './list.js';
...
describe('elementList', () => {
  test('toJSON returns elements array', () => {
    const result = elementList.toJSON(makeCtx()) as { elements: { tagName: string; description?: string }[] };
    expect(result.elements).toHaveLength(1);
    expect(result.elements[0].tagName).toBe('bp-button');
    expect(result.elements[0].description).toBe('A button component');
  });

  test('toMarkdown returns formatted string', () => {
    const md = elementList.toMarkdown(makeCtx());
